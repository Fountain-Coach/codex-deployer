//
// SearchGroupedHit.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

public struct SearchGroupedHit: Sendable, Codable, ParameterConvertible, Hashable {

    public var found: Int?
    public var groupKey: [JSONValue]
    /** The documents that matched the search query */
    public var hits: [SearchResultHit]

    public init(found: Int? = nil, groupKey: [JSONValue], hits: [SearchResultHit]) {
        self.found = found
        self.groupKey = groupKey
        self.hits = hits
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case found
        case groupKey = "group_key"
        case hits
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(found, forKey: .found)
        try container.encode(groupKey, forKey: .groupKey)
        try container.encode(hits, forKey: .hits)
    }
}

